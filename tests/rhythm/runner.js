...
test("gap answers can never touch scored rows (write-path audit)", () => {
  const src = fs.readFileSync(path.join(__dirname, "../../netlify/functions/checkin-prompt.js"), "utf8");
  const answerBlock = src.slice(src.indexOf("body.answer"), src.indexOf("compose path"));
